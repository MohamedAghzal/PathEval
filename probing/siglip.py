...
parser.add_argument("--model_name", type=str, default="google/siglip-base-patch16-224", help="Name of the model")
...
class ProbingModel(nn.Module):
    def __init__(self, model):
        ...
        self.model = model
        self.projection_dim = model.vision_model.config.hidden_size
        self.probing_layer = nn.Linear(self.projection_dim * 2, 1)  # Binary classification

    def forward(self, image1, image2):
        image1_features = self.model.get_image_features(image1)
        image2_features = self.model.get_image_features(image2)
