...
parser.add_argument("--model_name", type=str, default="facebook/dino-vitb8", help="Name of the model")
...
class ProbingModel(nn.Module):
    def __init__(self, model):
        ...
        self.model = model
        self.projection_dim = model.config.hidden_size
        self.probing_layer = nn.Linear(self.projection_dim * 2, 1)

    def forward(self, image1, image2):
        image1_features = self.model(image1).last_hidden_state.mean(dim=1)
        image2_features = self.model(image2).last_hidden_state.mean(dim=1)
...
probing_model = ProbingModel(model).to(device)

criterion = nn.BCELoss()
optimizer = torch.optim.Adam(probing_model.parameters() if req_grad else probing_model.probing_layer.parameters(), lr=0.00001)

output_answers = []
best_cosine_similarity = 1.5
for epoch in range(30): 
    ...
    with open(f"{model_name.replace('/', '_')}_probe_unfreeze_outputs.txt", 'a') as f:
        ...
        f.write(f"Accuracy: {accuracy:.2f}%, Loss: {avg_loss:.4f}, Avg. Similarity: {avg_cosine_similarity:.4f}\n")
    
    print(f"Epoch {epoch + 1}: Accuracy: {accuracy:.2f}%, Loss: {avg_loss:.4f}, Avg. Similarity: {avg_cosine_similarity:.4f}")

    if avg_cosine_similarity < best_cosine_similarity:
        best_cosine_similarity = best_cosine_similarity
        torch.save(probing_model.state_dict(), f"{model_name.replace('/', '_')}_best_model_{req_grad}.pth")
        print(f"Best model saved with accuracy: {best_cosine_similarity:.2f}%")

    output_filename = f"{model_name.replace('/', '_')}_{req_grad}_probe_outputs_500.json"
